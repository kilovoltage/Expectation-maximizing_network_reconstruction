...
colors = ["blue","gold","cyan","deepskyblue","darkviolet","dodgerblue","orange","beige","turquoise"]
nx.draw_networkx(ws,node_color=random.choices(colors,k=n),edge_color='grey',node_size=[10*ws.degree[i] for i in ws.nodes],with_labels = True)
plt.axis('off')
plt.savefig('watts_graph.svg',format='svg')
plt.show()
plt.close()
G=ws
...
plt.ylabel("F1") #Y轴标签
plt.savefig('watts_times.svg',format='svg')
plt.show()
plt.close()
...
plt.ylabel("F1") #Y轴标签
plt.savefig('watts_rho.svg',format='svg')
plt.show()
plt.close()