...
for ii in range(3):
    ...
    plt.plot(listM, listF11[ii], marker='*',c=color[ii], ms=10,mec='r', mfc='w',label=f'k1=12,two-body')
    plt.plot(listM, listF12[ii], marker='o',c=color[ii],ms=10,label=f'k2={c[ii]},three-body')

plt.legend()  # 让图例生效
plt.xlabel(u"times") #X轴标签
plt.ylabel("F1") #Y轴标签
plt.savefig('main_k2.svg',format='svg')
plt.show()
plt.close()
